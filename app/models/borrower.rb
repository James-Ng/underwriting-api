class Borrower < ActiveRecord::Base
  ...
  def check_property_eligibility
    errors.add(:property_type, I18n.t('errors.borrower.property_eligibility')) unless self.property_type && self.property_type.is_active
  end
  ...
  def housing_expense_ratio
    ...
    if ratio > 0.28
      errors.add(:base, I18n.t('errors.borrower.high_ratio'))
    end
  end

  # formula to calc total_income
  def total_income
    self.base_income.to_i + self.rental_income.to_i + self.commission.to_i
  end

  # formula to calc housing_expense
  def housing_expense
    self.mortgage_payment.to_f + self.mortgage_insurance.to_f + self.homeowner_insurance.to_f + self.property_tax.to_f + self.HOA_due.to_f
  end
